use agent_core::{InvoicePipeline, InvoiceData, PlanTier, TenantContext};
...
#[derive(Parser, Debug)]
#[command(name = "invoice-demo", about = "Extract structured data from an invoice image")]
struct Args {
    ...
    if !inv.line_items.is_empty() {
        ...
        for item in &inv.line_items {
            let total = item.total
                ...
                .unwrap_or_else(|| "—".into());
            println!("  {:.<45} {}", format!("{} ", item.description).dimmed(), total.bold());
        }
    }
...
fn field(label: &str, val: &Option<impl std::fmt::Display>) {
    if let Some(v) = val {
        println!("  {:15} {}", format!("{}:", label).dimmed(), v.to_string().bold());
    }
}

fn money_field(label: &str, val: Option<f64>, currency: Option<&str>) {
    if let Some(v) = val {
        println!("  {:15} {}", format!("{}:", label).dimmed(), format_money(v, currency).bold());
    }
}
