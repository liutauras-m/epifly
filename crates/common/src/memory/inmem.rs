...
impl ThreadStore for InMemoryThreadStore {
    async fn create(
        ...
        };
        {
            ...
            threads.insert((tenant_id.to_owned(), thread_id.clone()), thread.clone());
        }
        {
            let mut msgs = self.messages.lock().unwrap();
            msgs.insert((tenant_id.to_owned(), thread_id.clone()), initial_messages);
        }
        Ok(thread)
    }
...
impl AuditStore for InMemoryAuditStore {
    ...
    async fn list(&self, tenant_id: &str, limit: usize) -> crate::error::Result<Vec<AuditEvent>> {
