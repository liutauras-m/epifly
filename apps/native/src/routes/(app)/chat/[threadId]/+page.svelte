<script lang="ts">
  ...
  import { AppSafeArea, ChatBreadcrumb, ChatComposer, ChatMessageList, ChatEmptyState, ChatStreamStatus, WorkspaceFilingSuggestion } from "@epifly/ui";
  ...
  {#if threadNode.virtualPath}
    <div class="flex shrink-0 items-center gap-2 border-b border-border/40 px-4 py-1.5">
      ...
      {#if !contextDetached && activeContextNodeId}
        <button
          type="button"
          class="ml-auto flex shrink-0 items-center gap-1 rounded-full bg-muted/60 px-2 py-0.5 text-[0.65rem] text-muted-foreground/70 hover:bg-muted hover:text-muted-foreground active:opacity-80"
          title="Using context from {threadNode.placeName ?? 'workspace'} — tap to detach"
          onclick={() => (contextDetached = true)}
          aria-label="Context: {threadNode.placeName ?? 'Workspace'} — tap to detach"
        >
      ...
      {:else}
        <button
          type="button"
          class="ml-auto flex shrink-0 items-center gap-1 rounded-full bg-muted/30 px-2 py-0.5 text-[0.65rem] text-muted-foreground/40 hover:bg-muted/60 hover:text-muted-foreground/70 active:opacity-80"
          title="No folder context — tap to re-attach"
          onclick={() => (contextDetached = false)}
          aria-label="No context — tap to re-attach"
        >
