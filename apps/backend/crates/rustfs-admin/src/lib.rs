...
pub use iam::{IamCreds, deprovision_tenant, provision_tenant};
...
impl RustFsAdminClient {
    ...
    async fn s3_request(
        ...
        let req = signing::SigningRequest {
        ...
        let signed = signing::sign_request(&ctx, &req);
    ...
    async fn admin_request(
        ...
        let resp = self
            .s3_request(method, &full_path, query, &BTreeMap::new(), payload)
            .await?;
    ...
    pub async fn put_cors(&self, origins: &[String]) -> Result<()> {
        ...
        headers.insert("content-type".into(), "application/xml".into());
        let resp = self
            .s3_request(Method::PUT, &path, "cors", &headers, Bytes::from(xml))
            .await?;
        let s = resp.status();
        if s.is_success() {
            Ok(())
...
<ServerSideEncryptionConfiguration xmlns="http://s3.amazonaws.com/doc/2006-03-01/">
  <Rule><ApplyServerSideEncryptionByDefault><SSEAlgorithm>AES256</SSEAlgorithm></ApplyServerSideEncryptionByDefault></Rule>
        ...
        if !resp.status().is_success() {
            ...
            tracing::warn!(
                bucket_name,
                "put_bucket_encryption on new bucket failed: {t}"
            );
        }
  ...
  <Rule>
    ...
    <Expiration><Days>7</Days></Expiration>
        ...
        if !resp.status().is_success() {
            ...
            tracing::warn!(
                bucket_name,
                "put_lifecycle on new bucket failed (non-fatal): {t}"
            );
        }
    ...
    pub async fn create_service_account(
        ...
        if !s.is_success() {
            bail!(
                "create_service_account failed: {s} — {}",
    ...
    pub async fn create_bucket_scoped_service_account(
        ...
        if !s.is_success() {
            bail!(
                "create_bucket_scoped_service_account failed: {s} — {}",
                String::from_utf8_lossy(&body_bytes)
            );
        }
    ...
    pub async fn delete_service_account(&self, access_key: &str) -> Result<()> {
        ...
        if s.is_success() || s == StatusCode::NOT_FOUND {
            ...
            bail!(
                "delete_service_account failed: {s} — {}",
    ...
    pub async fn list_service_accounts(&self, user: &str) -> Result<Vec<String>> {
        ...
        if !s.is_success() {
            bail!(
                "list_service_accounts failed: {s} — {}",
                String::from_utf8_lossy(&body)
            );
        }
        #[derive(Deserialize)]
...
fn parse_version_list(xml: &str) -> Vec<(String, String, u64, bool)> {
    ...
    for block in xml.split("<Version>").skip(1) {
        ...
        let inner = &block[..end];
        let version_id = extract_xml_text(inner, "VersionId")
            .unwrap_or_default()
            .to_string();
        let last_modified = extract_xml_text(inner, "LastModified")
            .unwrap_or_default()
            .to_string();
...
mod tests {
    ...
    fn encrypt_admin_body_wire_layout() {
        ...
        assert_eq!(
            enc[32], 0x00,
            "byte[32] must be alg_id 0x00 (Argon2id+AES-GCM), got {}",
            enc[32]
        );
    }
