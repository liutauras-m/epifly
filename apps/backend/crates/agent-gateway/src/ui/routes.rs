...
pub fn ui_router() -> Router<Arc<AppState>> {
    let admin_routes = Router::new()
        ...
        .route(
            "/super-admin/new",
            get(super_admin::new_form).post(super_admin::create),
        )
        .route(
            "/super-admin/reload-all",
            post(super_admin::reload_all_caps),
        )
        .route(
            "/super-admin/{name}",
            get(super_admin::detail).post(super_admin::update),
        )
        .route(
            "/super-admin/{name}/toggle",
            post(super_admin::toggle_enabled),
        )
