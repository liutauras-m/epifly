...
pub async fn login_post(jar: CookieJar, Form(form): Form<LoginForm>) -> Response {
    ...
    // Determine role: super_admin if name matches SUPER_ADMIN_EMAILS (comma-separated).
    let super_admin_emails = std::env::var("SUPER_ADMIN_EMAILS").unwrap_or_default();
    let role = if super_admin_emails
        .split(',')
        .any(|e| e.trim().eq_ignore_ascii_case(name))
    {
        "super_admin"
    } else {
        "user"
    };
    let token = session::sign(name, &form.plan, role);
