...
use super::{AgentProvider, ProviderError, ProviderEvent, ProviderEventSink, ProviderRequest, ProviderResponse};
...
impl NativeAnthropicProvider {
    async fn send_with_retry(
            ...
            match response {
                ...
                Err(e) => {
                    ...
                    if attempt == MAX_UPSTREAM_ATTEMPTS {
                        record_retry_exhausted(model);
                        return Err(ProviderError::Transport(format!("Anthropic request failed: {e}")));
                    }
        ...
        Err(ProviderError::Transport("upstream retry loop exited unexpectedly".into()))
    }
}
...
impl AgentProvider for NativeAnthropicProvider {
    async fn complete(
        ...
        Ok(ProviderResponse {
            ...
            stop_reason: parsed["stop_reason"].as_str().unwrap_or("end_turn").to_string(),
    ...
    async fn stream_events(
        ...
        if !resp.status().is_success() {
            ...
            let body_msg = serde_json::from_str::<Value>(&raw)
                ...
                .unwrap_or(raw);
            return Err(ProviderError::UpstreamHttp { status, body: body_msg });
        }
